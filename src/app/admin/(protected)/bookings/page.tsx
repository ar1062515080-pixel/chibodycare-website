...
export default async function BookingsPage({ searchParams }: { searchParams: Promise<{ date?: string; location?: string; status?: string }> }) {
  ...
  const { data: locationRows } = await supabase.from("locations").select("id,name").order("name");
  const locations = locationRows ?? [];
  let query = supabase.from("bookings").select("id,reference,customer_name,customer_phone,customer_email,start_at,end_at,status,locations(name),services(name),therapists(display_name)").order("start_at", { ascending: false }).limit(200);
  if (params.date) query = query.gte("start_at", `${params.date}T00:00:00`).lt("start_at", `${params.date}T23:59:59`);

...
export default async function VouchersPage({ searchParams }: { searchParams: Promise<{ location?: string }> }) {
  ...
  return <div>
    ...
    <div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><h1 className="font-serif text-4xl text-brown-900">{tr(locale, "Voucher sales / use", "礼券售卖/使用")}</h1><p className="mt-2 text-sm text-brown-700/60">{tr(locale, "Sell, redeem and trace every voucher from one ledger.", "在同一台账中完成礼券售卖、使用及历史追踪。")}</p></div><form className="flex items-end gap-2"><label className="text-xs font-medium">{tr(locale, "Studio", "门店")}<select name="location" defaultValue={locationId} className="mt-1 block min-w-64 rounded-xl border border-sand-200 bg-white px-3 py-2 text-sm">{locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select></label><SubmitButton pendingLabel={tr(locale, "Loading…", "加载中…")} className="rounded-xl bg-brown-900 px-4 py-2.5 text-sm text-white">{tr(locale, "View", "查看")}</SubmitButton></form></div>

    <div className="mt-7 grid gap-5 xl:grid-cols-2">
      <section className="rounded-3xl border border-sand-200 bg-cream-50 p-5 shadow-sm md:p-6">
        <div className="flex items-center justify-between"><div><p className="text-xs uppercase tracking-[0.16em] text-gold-dark">01 · {tr(locale, "Sell", "售出")}</p><h2 className="mt-1 font-serif text-2xl">{tr(locale, "Sell a gift voucher", "礼券售出")}</h2></div><span className="rounded-full bg-sage-100 px-3 py-1 text-xs text-sage-800">{today}</span></div>
        <form action={saveGiftVoucherSale} className="mt-5 space-y-4">
          <input type="hidden" name="location_id" value={locationId} /><input type="hidden" name="sale_date" value={today} />

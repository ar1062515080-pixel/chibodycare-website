...
export default async function LocationsPage() {
  ...
  return <div>
    <p className="text-xs uppercase tracking-[0.18em] text-gold-dark">{tr(locale, "Locations", "门店管理")}</p>
    ...
    <div className="mt-8 space-y-5">{locations.map((location) => {
      ...
      return <form action={saveLocation} key={location.id} className="rounded-3xl border border-sand-200 bg-cream-50 p-6 shadow-sm">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <label className="text-sm">{tr(locale, "Phone", "电话")}<input name="phone" defaultValue={location.phone} required className="mt-1 w-full rounded-xl border border-sand-200 px-3 py-2" /></label>
          <label className="text-sm">{tr(locale, "Address", "地址")}<input name="address" defaultValue={location.address || ""} className="mt-1 w-full rounded-xl border border-sand-200 px-3 py-2" /></label>
        </div>
        <label className="mt-4 flex gap-2 text-sm"><input type="checkbox" name="active" defaultChecked={location.active} />{tr(locale, "Active location", "启用该门店")}</label>
        <details className="mt-4"><summary className="cursor-pointer text-sm font-medium text-sage-700">{tr(locale, "Edit opening hours", "编辑营业时间")}</summary><div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{days.map((day) => {
          const entry = hours.get(day); const dayLabel = dayLabels[day][locale];

...
export async function saveTherapist(formData: FormData) {
  ...
  if (!payload.display_name || !payload.internal_name || !locationIds.length) return;
  const result = id
  ...
  const { error: removeLocationError } = await supabase
    .from("location_therapists")
    .delete()
    .eq("therapist_id", therapistId);
  if (removeLocationError) throw new Error(removeLocationError.message);
  const { error: locationError } = await supabase.from("location_therapists").insert(
    locationIds.map((locationId) => ({ location_id: locationId, therapist_id: therapistId })),
  );

  ...
  return <div>
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      {bookingLocations.map((location) => {
        ...
        return <button key={location.id} type="button" onClick={() => dispatch({ type: "SET_LOCATION", locationId: location.id })} aria-pressed={selected} className={cn("rounded-2xl border p-4 text-left transition-all", selected ? "border-gold-dark bg-champagne/45 shadow-sm" : "border-sand-200 bg-cream-50 hover:border-gold-light")}>

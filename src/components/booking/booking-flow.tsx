    ...
    <div className="grid gap-8 lg:grid-cols-[1fr_22rem]">
      <div>
        <div className="rounded-3xl border border-sand-100 bg-cream-50 p-6 shadow-sm sm:p-8">
          ...
          {state.step === 2 ? <><StepDateTime />{state.startAt ? <div className="mt-10 border-t border-sand-100 pt-10"><StepProfessional /></div> : null}</> : null}
        ...
        {!isConfirmation && state.step !== 1 ? <div className="mt-6 flex items-center justify-between gap-4">
          <button type="button" onClick={() => { setShowContactErrors(false); dispatch({ type: "BACK" }); }} className="inline-flex items-center gap-2 rounded-full px-5 py-3 text-sm font-medium text-brown-800 transition-colors hover:bg-sand-50">← Back</button>
          <div className="flex flex-col items-end gap-1.5">
            <button type="button" onClick={handleNext} disabled={submitting || (!canProceed && state.step !== 4)} className="inline-flex items-center gap-2 rounded-full bg-sage-600 px-7 py-3 text-sm font-medium text-cream-50 shadow-sm transition-all hover:bg-sage-700 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-60">

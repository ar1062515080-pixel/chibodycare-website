...
export function BookingFlow() {
  ...
  const canProceed = (() => {
    switch (state.step) {
      ...
      case 2: return Boolean(state.dateKey && state.time && state.professionalId);
      case 3: return state.serviceIds.length > 0;
      case 4: return isContactValid(state.contact);
      default: return true;
    }
  })();
  ...
  return <div className="container-page max-w-full py-8 sm:py-14">
    {!isConfirmation ? <div className="mb-6 min-w-0 rounded-3xl border border-sand-100 bg-cream-50 p-5 shadow-sm sm:mb-8 sm:p-6"><StepIndicator /></div> : null}
    <div className={cn("grid min-w-0 gap-6 sm:gap-8", isConfirmation ? "mx-auto max-w-3xl" : "lg:grid-cols-[minmax(0,1fr)_22rem]")}>
      <div className="min-w-0">
        <div className="min-w-0 rounded-3xl border border-sand-100 bg-cream-50 p-5 shadow-sm sm:p-8">
          ...
          {state.step === 2 ? <><StepDateTime />{state.startAt ? <div className="mt-8 border-t border-sand-100 pt-8 sm:mt-10 sm:pt-10"><StepProfessional /></div> : null}</> : null}
        ...
        {!isConfirmation && state.step !== 1 ? <div className="mt-6 flex items-center justify-between gap-4">
          <button type="button" onClick={() => { setShowContactErrors(false); dispatch({ type: "BACK" }); }} className="inline-flex items-center gap-2 rounded-full px-4 py-3 text-sm font-medium text-brown-800 transition-colors hover:bg-sand-50 sm:px-5">← Back</button>
          <div className="flex flex-col items-end gap-1.5">
            <button type="button" onClick={handleNext} disabled={submitting || (!canProceed && state.step !== 4)} className="inline-flex items-center gap-2 rounded-full bg-sage-600 px-5 py-3 text-sm font-medium text-cream-50 shadow-sm transition-all hover:bg-sage-700 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-60 sm:px-7">
              {submitting ? "Saving booking…" : state.step === 4 ? "Confirm booking" : "Continue →"}
            </button>
            {!canProceed && stepHints[state.step] ? <p className="max-w-[12rem] text-right text-xs text-brown-700/60 sm:max-w-none">{stepHints[state.step]}</p> : null}

...
export function StepDateTime() {
  ...
  return <div>
    ...
    <div className="rounded-3xl border border-sand-200 bg-cream-50/90 p-4 shadow-sm sm:p-5">
      <label className="block text-sm font-medium text-brown-800" htmlFor="booking-date">Select a date</label>
      <input
        id="booking-date"
        type="date"
        min={minDateKey}
      ...
      />
      <p className="mt-3 text-xs leading-relaxed text-brown-700/65">Choose any future date. Available start times will appear below after the daily roster is checked.</p>
    </div>
    <div className="mt-6">
      {!selectedDate ? <p className="rounded-2xl border border-dashed border-sand-200 px-4 py-8 text-center text-sm text-brown-700/60">Select a date to see available start times.</p>
        ...
        : <><p className="mb-3 text-sm font-medium text-brown-800">Available start times · {formatLongDate(selectedDate)}</p><div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">

-- Keep public bookings tolerant of blank optional email values.
-- This repeats the current allocator so production databases that missed one
-- of the earlier patches still accept empty email and use therapist capabilities.
...
begin
  ...
  for v_roster in
    ...
    where dr.location_id = p_location_id
      ...
      and dr.active
      and (p_therapist_id is null or dr.therapist_id = p_therapist_id)
      and (p_therapist_id is null or t.public_display or public.is_admin())
  ...
  loop
    begin
      ...
      insert into public.bookings (
        reference, customer_name, customer_phone, customer_email, location_id,
        service_id, therapist_id, daily_roster_id, start_at, end_at, status, notes
      ) values (
        v_reference, p_customer_name, p_customer_phone, p_customer_email,
        p_location_id, p_service_id, v_roster.therapist_id, v_roster.id,
        p_start_at, v_end_at, 'pending', coalesce(p_notes, '')
